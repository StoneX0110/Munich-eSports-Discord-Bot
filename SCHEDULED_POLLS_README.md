...
|--------|-------------|---------|
| `/scheduled-poll create <role> <weekday>` | Erstellt eine wöchentliche Umfrage für `role`. Die Umfrage wird in dem Kanal gepostet, in dem der Befehl ausgeführt wird. | `/scheduled-poll create role:@CS Main Team weekday:Mittwoch` würde jeden Mittwoch das CS Main Team pingen und nach Terminverfügbarkeit für die kommende Woche (ab Montag) fragen.|
| `/scheduled-poll create <role> <weekday> <reminder_weekday> <reminder_hour>` | Erstellt eine wöchentliche Umfrage mit Reminder. `reminder_hour` ist eine Stunde im 24h-Format von `0` bis `23`. | `/scheduled-poll create role:@CS Main Team weekday:Mittwoch reminder_weekday:Sonntag reminder_hour:18` macht das gleiche wie oben, nur dass jeden Sonntag um 18 Uhr alle aus dem CS Main Team gepingt werden, die bis dahin noch nicht abgestimmt haben.|
| `/scheduled-poll list` | Zeigt alle eingerichteten wiederkehrenden Umfragen mit ID, Rolle, Kanal, Wochentag und Reminder an. | `/scheduled-poll list` |
...
|------------|-----------|
| `weekday` | Wochentag, an dem die neue Umfrage automatisch gepostet wird. |
| Posting-Zeit | Die Umfrage wird in der 08:00-Uhr-Stunde in `Europe/Berlin` gepostet. |
| Zielwoche | Die Umfrage fragt immer die Verfügbarkeit für die folgende Woche ab, beginnend mit Montag. |
